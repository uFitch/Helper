...
def main():
    ...
    while True:
        ...
        print(
            '1.  "load" to load AddressBook and NotesBook\n2.  "new" to create new Book\n3.  "exit"/"close" to close '
            'application:')
        command = str(input())
        if command == "load" or command == "дщфв" or command == "1":
            print(r'Please write the full path to file with AddressBook and NotesBook. Example: "d:\test\book.txt":')
            path = str(input())
            try:
...
def add():
    ...
    while True:
        ...
        if decision == 'y' or decision == 'yes' or decision == 'нуі' or decision == 'н' or decision == 'да' or decision == 'д':
            print(
                'Input Birthday. Expected day.month.year(Example:25.12.1970). If year of birth is not known, type 1111')
            birthday = str(input())
            try:
...
def change():
    ...
    if len(result) > 0 and len(result) is not None:
        show_find(result)
        print(100 * "_")
        print(
            '1.   To change Name: type "name".\n2.   To change Phone: type "phone".\n3.   To change Birthday: type '
            '"birthday".\n4.   To change Address: type "address".\n5.   To change E-mail: type "email".\n6.   To '
            'change Tags: type "tags"\n7.   To exit: type "exit".\n')
        decision = str(input())
        decision = decision.lower()
        if decision == 'name' or decision == 'тфьу' or decision == '1':
            ...
            if len(result) > 1:
                ...
                print(f'{old_name} not in Address Book')
            ...
            print(
                'Type birthday you want to change. Expected day.month.year(Example:25.12.1970). If there is no '
                'birthday - just press "enter".')
            if len(result) > 1:
                print(f"I've found {len(result)} notes with this Name")
                show_find(result)
                ...
                for i in result:
                    if i["Id"] == del_input:
                        old_name = str(input())
                        print(
                            'Type new birthday. Expected day.month.year(Example:25.12.1970). If year of birth is not '
                            'known, type 1111')
                        new_name = str(input())
                        try:
...
def clean_folder():
    ...
    print(100 * "_")
    print('Please enter path to clean and structure.')
...
def birthday():
    ...
    print(100 * '_')
    print(
        "1.   If you want to find, who'll have birthday in exact date TYPE 1.\n2.   If you need to know who'll have "
        "birthday in period of time TYPE 2.\n3.   If you need to know how many days to somebody's birthday TYPE "
        "3.\n4.   Type 'exit' to exit")
    decision = int(input())
    result = []
    if decision == 1:
        ...
        for i in book:
            if i["Birthday"] != 0 and i["Birthday"] is not None:
        ...
        if len(result) > 1:
            ...
            for i in result:
                if i["Id"] == id_input:
                    ...
                    print(
                        f'{i["Name"]} from your Addressbook will have birthday in {days} days. Do not forget to '
                        f'congratulate!')
        elif len(result) == 1:
            for i in result:
                days = days_to_birthday(i['Birthday'])
            ...
            print(
                f'No information about birthday. Please enter valid information using command "change" or add new '
                f'person to Addressbook')
...
def show_find(v_list):
    ...
    print(145 * '_')
    print(
        '| ID  |           Name           |     Phones      |  Birthday  |           Address            |             '
        ' E-mail            |       Tags     |')
    print(145 * '-')
    for i in v_list:
        print(
            f'|{i["Id"]:<5}| {i["Name"]:<25}| {i["Phones"][0] if len(i["Phones"]) >= 1 else " ":<15} | {i["Birthday"] if i["Birthday"] else " ":<11}|{i["Address"] if i["Address"] else " ":<30}|  {i["E-mail"] if i["E-mail"] else " ":<30}| {i["Tags"] if i["Tags"] else " ":<15}|')
        if len(i["Phones"]) > 1:
...
def find_note():
    ...
    if result:
        print(result)
        return "The search is successfully finished"
    else:
...
ANSWEARS = {'add': add, 'ad': add, '+': add, 'фвв': add, 'change': change, 'срфтпу': change, 'close': exit,
            'exit': exit, 'учше': exit, 'clear': clear, 'сдуфк': clear,
            'find': find, 'аштв': find, 'help': help_func, 'рудз': help_func, 'хелп': help_func, 'save': save,
            'іфму': save, 'ыфму': save, 'show': show1, 'ырщц': show1, 'ірщц': show1,
            'delete': delete, 'del': delete, 'вуд': delete, 'вудуеу': delete, 'birthday': birthday,
            'ишкервфн': birthday, 'clean': clean_folder, 'сдуфт': clean_folder,
            'add note': add_note, 'фвв тщеу': add_note, 'delete note': delete_note, 'вудуеу тщеу': delete_note,
            'edit note': edit_note, 'увше тщеу': edit_note,
            'find note': find_note, 'аштв тщеу': find_note, 'sort notes': sort_notes, 'ыщке тщеуы': sort_notes,
